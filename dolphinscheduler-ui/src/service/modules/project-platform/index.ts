...
export async function getPlatformRest(platform: String, rest: PlatformRestEnum, clusterId?: Number, nodeId?: Number, taskType?: String): Promise<Map<string, Object>> {
    const response = await axios({
    ...
    return response as unknown as Map<string, Object>
}
...
// 创建项目节点参数
export async function createProjectNodeParameter(projectCode: number, nodeCode: number, parameterName: string, parameterValue: string): Promise<ProjectNodeParameter> {
    const response = await axios({
        url: `/projects/${projectCode}/project-node/${nodeCode}/parameters`,
        method: 'post',
        data: {
            parameterName,
            parameterValue
...
// 更新项目节点参数
export async function updateProjectNodeParameter(projectCode: number, nodeCode: number, code: number, parameterName: string, parameterValue: string, description?: string): Promise<ProjectNodeParameter> {
    const response = await axios({
        url: `/projects/${projectCode}/project-node/${nodeCode}/parameters/${code}`,
        method: 'put',
        data: {
            parameterName,
            parameterValue,
...
// 删除项目节点参数
export async function deleteProjectNodeParameter(projectCode: number, nodeCode: number, code: number): Promise<boolean> {
    const response = await axios({
        url: `/projects/${projectCode}/project-node/${nodeCode}/parameters/${code}`,
        method: 'delete'
    });
...
// 批量删除项目节点参数
export async function batchDeleteProjectNodeParameters(projectCode: number, nodeCode: number, codes: string): Promise<boolean> {
    const response = await axios({
        url: `/projects/${projectCode}/project-node/${nodeCode}/batch/parameters`,
        method: 'delete',
        data: {
            codes
        }
...
// 查询项目节点参数列表
export async function queryProjectNodeParametersList(projectCode: number, nodeCode: number): Promise<ProjectNodeParameter[]> {
    const response = await axios({
        url: `/projects/${projectCode}/project-node/${nodeCode}/parameters`,
        method: 'get'
    });
